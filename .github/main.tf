resource "google_storage_bucket" "gcs_bucket" {
    name = "stack-state-gcs-datapipeline01-98239174"
    location = var.region
}
...
module "bucket-raw" {
  ...
  name       = "data-pipeline-test-combustiveis-brasil-raw"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-curated" {
  ...
  name       = "data-pipeline-test-combustiveis-brasil-curated"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-pyspark-tmp" {
  ...
  name       = "data-pipeline-test-combustiveis-brasil-pyspark-tmp"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-pyspark-code" {
  ...
  name       = "data-pipeline-test-combustiveis-brasil-pyspark-code"
  project_id = var.project_id
  location   = var.region
}